...
class AIcCatalogPersistentSchema:
    ...
    def __post_init__(self) -> None:
        if not self.schema_id or self.write_version < 1:
            raise ValueError("catalog persistent schema requires schema_id/write_version")
        if self.kind is AInCatalogPersistentSchemaKind.COMPONENT_CONFIGURATION:
            if self.provider_id is not None or self.entity_type_id is not None:
                raise ValueError("component configuration schema must not declare provider_id/entity_type_id")
        elif self.kind is AInCatalogPersistentSchemaKind.PROVIDER_CONFIGURATION:
            if not self.provider_id or self.entity_type_id is not None:
                raise ValueError("provider configuration schema requires provider_id and forbids entity_type_id")
        elif self.kind is AInCatalogPersistentSchemaKind.ENTITY_EXTENSION:
            if not self.entity_type_id or self.provider_id is not None:
                raise ValueError("entity extension schema requires entity_type_id and forbids provider_id")

    @property
    def identity(self) -> tuple[str, str | None]:
        if self.kind is AInCatalogPersistentSchemaKind.PROVIDER_CONFIGURATION:
            return self.kind.value, self.provider_id
        if self.kind is AInCatalogPersistentSchemaKind.ENTITY_EXTENSION:
            return self.kind.value, self.entity_type_id
