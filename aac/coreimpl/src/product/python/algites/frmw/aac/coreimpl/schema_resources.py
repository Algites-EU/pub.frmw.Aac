...
def read_core_schema(resource_name: str) -> str:
    ...
    try:
        return resources.files("algites.frmw.aac.coreintf").joinpath(relative).read_text(encoding="utf-8")
    except (ModuleNotFoundError, FileNotFoundError):
        here = Path(__file__).resolve()
        for parent in here.parents:
            source = parent / "aac/coreintf/src/product/schema/algites/frmw/aac/coreintf" / relative
            if source.is_file():
