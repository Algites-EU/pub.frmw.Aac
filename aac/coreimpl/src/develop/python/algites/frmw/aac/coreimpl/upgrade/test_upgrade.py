...
from algites.frmw.aac.coreimpl.errors import AIxUpgradeError
...
def _write_candidate(root: Path, package: str, *, version: int, broken: bool = False) -> None:
    ...
    (pkg / "provider.py").write_text(
        ...
        "from algites.frmw.aac.coreintf.observation import AIiObservationProvider, AIcObservationOutput\n"
    ...
    schema_text = resources.files("algites.frmw.aac.simpleaudit").joinpath("schemas/simpleaudit-config_1.json").read_text(encoding="utf-8")
...
'''
    ...
    with ZipFile(path, "w") as archive:
        ...
        archive.writestr("sameupgrade/capability/same_1.yml", contract)
...
def test_stored_package_transaction_commits_selection_and_obsoletes_old_artifact(tmp_path: Path):
    ...
    from algites.frmw.aac.coreintf.packages import AIcPackageCandidate, AIcPackageStoreLayout, AInStoredPackageState
